...
var fs = require('fs'),
    ...
    formats = require('../../lib/nconf/formats'),
    File = require('../../lib/nconf/stores/file'),
    data = require('../fixtures/data').data;

var fixturesDir = path.join(__dirname, '..', 'fixtures');

describe('nconf/stores/file', function () {
  it('should be exposed correctly on nconf', function () {
    assume(File).equals(nconf.File);
  })

  it('should have the correct methods defined', function () {
    var fullpath = path.join(fixturesDir, 'store.json');
    var store = nconf.File({ file: fullpath });
    var props = Object.keys(store);
    ['type', 'file', 'dir', 'format', 'spacing']
      .forEach(function (key) {
        assume(props).includes(key);
      });

    assume(store.readOnly).equals(undefined);
    assume(store.type).equals('file');
    assume(store.file).equals(fullpath);
    assume(store.dir).is.a('string');
    assume(store.format).equals(formats.json);
    assume(store.spacing).equals(2);
  });
  ...
  describe('load()', function () {
    ...
    it('should parse a valid UTF8 JSON file');
    it('should parse a valid UTF8 JSON file with BOM');
    it('should error on a malformed JSON file');
  });
