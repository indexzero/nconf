...
var Env = exports.Env = function (options) {
  ...
  this.lowerCase = options.camelCase || options.lowerCase || false;
...
Env.prototype.loadEnv = function () {
  ...
  Object.keys(process.env).filter(function (key) {
    ...
    if (self.separator) {
      newKey = common.key.apply(common, newKey.split(self.separator));
    }
    if (self.camelCase) {
      newKey = common.key.apply(common, common.path(newKey).map(common.camelCase));
    } else if (self.lowerCase) {
      newKey = newKey.toLowerCase();
    }

    self.set(newKey, process.env[key]);
  });
