...
//
Provider.prototype.dump = function (callback) {
//
  // If there is no callback we can short-circuit into the default
  // logic for traversing stores.
  //
  if (!callback) {
    return this._execute('dump', 0, callback);
  }

  //
  // Otherwise the asynchronous dump
  //
  var current = 0,
  ...
  async.whilst(function () {
    ...
    if (!store.dump) return next();

    if (store.dump.length) {
      return store.dump(function (err, value) {
        if (err) {
          return next(err);
        }
